...
export function ResponseHistoryTable({ history }: ResponseHistoryTableProps) {
	...
	const activeAlarms = history.filter(
		(item) => item.respondedAt === "対応中...",
	);
	const pastHistories = history.filter(
		(item) => item.respondedAt !== "対応中...",
	);
					...
					{activeAlarms.map((alarm) => (
						...
						>
							<div className="flex items-center gap-2 font-bold text-base">
								...
								<span>
									【緊急対応中】{alarm.judgment}アラートが発生しています
								</span>
							</div>
						</div>
					))}
					...
					<div className="hidden md:block overflow-hidden rounded-xl border border-gray-200">
						<table className="w-full text-left border-collapse bg-white text-sm">
							...
							<caption className="sr-only">
								過去の転倒および誤検知の対応実績履歴一覧
							</caption>
							<thead>
								<tr className="bg-gray-50 border-b border-gray-200 text-gray-500 font-medium">
									<th scope="col" className="p-4 w-[160px]">
										発生日時
									</th>
									<th scope="col" className="p-4 w-[100px]">
										判定結果
									</th>
									<th scope="col" className="p-4 w-[100px]">
										対応時間
									</th>
									<th scope="col" className="p-4 w-[120px]">
										対応スタッフ
									</th>
							...
							<tbody className="divide-y divide-gray-100 text-gray-700">
								{pastHistories.map((item) => (
									...
									>
										<th
											scope="row"
											className="p-4 font-mono font-normal text-gray-500 whitespace-nowrap"
										>
											{item.createdAt}
										</th>
